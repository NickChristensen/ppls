...
describe('custom-fields:list', () => {
  ...
  it('respects page and page size flags', async () => {
    globalThis.fetch = async (input) => {
      ...
      return new Response(
        JSON.stringify({
          next: 'https://paperless.example.test/api/custom_fields/?page=3',
          results: [{'data_type': 'string', 'document_count': 1, id: 2, name: 'Page Two'}],
        }),
        {
          headers: {'Content-Type': 'application/json'},
          status: 200,
    ...
    const {stdout} = await runCommand('custom-fields:list --page 2 --page-size 1')
  ...
  it('supports sort ordering', async () => {
    ...
    await runCommand('custom-fields:list --sort name')
    const requestUrl = new URL(requests[0])
  ...
  it('respects page size overrides', async () => {
    globalThis.fetch = async (input) => {
      ...
      return new Response(
        JSON.stringify({
          ...
          results: [{'data_type': 'string', 'document_count': 3, id: 3, name: 'First Five'}],
        }),
        {
          headers: {'Content-Type': 'application/json'},
          status: 200,
    ...
    const {stdout} = await runCommand('custom-fields:list --page-size 5')
  ...
  it('renders a plain list when requested', async () => {
    globalThis.fetch = async () =>
      new Response(
        JSON.stringify({
          next: null,
          results: [{'data_type': 'string', 'document_count': 1, id: 10, name: 'VIN'}],
        }),
        {
          headers: {'Content-Type': 'application/json'},
          status: 200,
    ...
    const {stdout} = await runCommand('custom-fields:list --plain')
    expect(stdout.trim()).to.equal('[10] VIN (1 document)')
  })
})
