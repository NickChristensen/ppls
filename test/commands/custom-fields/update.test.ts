...
describe('custom-fields:update', () => {
  ...
  it('updates a custom field', async () => {
    ...
    const {stdout} = await runCommand(
      'custom-fields:update 9 --name "Receipt Link" --data-type select --option apple --option banana --plain',
    )
  ...
  it('returns the payload when json is enabled', async () => {
    ...
    const {stdout} = await runCommand('custom-fields:update 12 --name "Due Date" --json')
  ...
  it('requires options for select fields', async () => {
    ...
    const {error} = await runCommand('custom-fields:update 9 --data-type select')
  ...
  it('rejects options for non-select fields', async () => {
    ...
    const {error} = await runCommand('custom-fields:update 9 --data-type number --option apple')
