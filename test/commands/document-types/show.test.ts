...
describe('document-types:show', () => {
  ...
  it('shows document type details in plain output', async () => {
    ...
    const {stdout} = await runCommand('document-types:show 42 --plain')

    expect(stdout.trim()).to.equal('[42] Bill (3 documents)')
    expect(new URL(requests[0]).pathname).to.equal('/api/document_types/42/')
  })
  ...
  it('renders a table by default', async () => {
    ...
    const {stdout} = await runCommand('document-types:show 42')
