...
describe('tags:show', () => {
  ...
  it('shows tag details in plain output', async () => {
    ...
    const {stdout} = await runCommand('tags:show 42 --plain')

    expect(stdout.trim()).to.equal('[42] Inbox (3 documents)')
    expect(new URL(requests[0]).pathname).to.equal('/api/tags/42/')
  })
  ...
  it('renders a table by default', async () => {
    ...
    const {stdout} = await runCommand('tags:show 42')
